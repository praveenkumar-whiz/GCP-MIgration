...
# -------------------------------------------------------------------
logger = logging.getLogger("vm-abuse")
logger.setLevel(logging.INFO)


def abuse_scan_spec(spec: Dict[str, Any]) -> Dict[str, Any]:
    """
    GCP Compute Engine VM abuse scan.
    ...
    logger.info("Compute Engine VM Abuse Scan triggered")
    ...
    try:
        ...
        # Rule 0: Region validation - check if instances are in allowed regions
        ...
        # Rule 5: deletion_protection_enabled
        # Reference: https://cloud.google.com/compute/docs/reference/rest/v1/instances
        if "deletion_protection_enabled" in rules:
            required_deletion_protection = rules.get("deletion_protection_enabled")
            if not isinstance(required_deletion_protection, bool):
                return {
                    "status": "FAILED",
                    "action": action,
                    "reason": "InvalidRuleValue",
                    "detail": "deletion_protection_enabled must be a boolean (true or false)",
                }
            ...
            for instance in active_instances:
                ...
                # Check if deletion protection matches the required setting
                if deletion_protection != required_deletion_protection:
                    if required_deletion_protection:
                        abuse_message.append(
                            "Project {} | VM instance {} does not have deletion protection enabled.".format(
                                project_id, name
                            )
                        )
                        logger.warning(
                            "ABUSE DETECTED - Deletion protection not enabled | Project={} | Instance={} | Required=true | Actual={}".format(
                                project_id, name, deletion_protection
                            )
                        )
                    else:
                        abuse_message.append(
                            "Project {} | VM instance {} has deletion protection enabled when it should be disabled.".format(
                                project_id, name
                            )
                        )
                        logger.warning(
                            "ABUSE DETECTED - Deletion protection enabled | Project={} | Instance={} | Required=false | Actual={}".format(
                                project_id, name, deletion_protection
                            )
                        )
